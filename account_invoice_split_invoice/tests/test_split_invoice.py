...
class TestSplitInvoice(common.TransactionCase):
    ...
    def test_open_invoice(self):
        ...
        self.assertEqual(
            wizard.invoice_line_ids[0].invoice_id.date_invoice, original_invoice_date
        )
        self.assertEqual(invoice.date_invoice, original_invoice_date)
        self.assertEqual(
            wizard.invoice_line_ids[0].move_id.account_id.id, original_account
        )
        self.assertEqual(invoice.account_id.id, original_account)
        self.assertEqual(
            wizard.invoice_line_ids[0].move_id.journal_id.id, original_journal
        )
        self.assertEqual(invoice.journal_id.id, original_journal)
        self.assertEqual(
            original_amount, invoice_new.amount_total + invoice.amount_total
        )
    ...
    def test_draft_invoice(self):
        ...
        original_account = invoice.account_id.id
        original_journal = invoice.journal_id.id
        wizard.write({"invoice_line_ids": [(4, self.invoice_line_id11)]})
        invoice_new = wizard.split_invoice()
        invoice = invoice_obj.browse(self.move_id1)
        # Test if the lines have been exactly copied
        self.assertEqual(invoice.name, original_name)
        self.assertEqual(
            wizard.invoice_line_ids[0].move_id.partner_id.id, orginal_partner_id
        )
        self.assertEqual(invoice.partner_id.id, orginal_partner_id)
        self.assertEqual(
            wizard.invoice_line_ids[0].move_id.date_invoice, original_invoice_date
        )
        self.assertEqual(invoice.date_invoice, original_invoice_date)
        self.assertEqual(
            wizard.invoice_line_ids[0].move_id.account_id.id, original_account
    ...
    def _create_invoice(self, invoice_name):
        ...
        account_id = (
            ...
            .search([("internal_type", "=", "receivable")])[0]
            .id
        )
        invoice_obj = self.env["account.move"]
    ...
    def _create_invoice_line(self, description, amount, move_id):
        ...
        invl_obj = self.env["account.move.line"].with_context(journal_id=1)
