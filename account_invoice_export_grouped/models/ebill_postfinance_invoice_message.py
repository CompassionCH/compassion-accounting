...
class EbillPostfinanceInvoiceMessage(models.Model):
    ...
    def _get_template_yb(self, jinja_env):
        if self.invoice_id:
            return super(EbillPostfinanceInvoiceMessage, self)._get_template_yb(
                jinja_env
            )
    ...
    def _get_payload_params_yb(self):
        if self.invoice_id:
            ...
            partner_bank_id = (
                self.grouped_invoices[0].partner_bank_id
                if len(self.grouped_invoices) > 0
                else False
            )
            partner_shipping_id = (
                self.grouped_invoices[0].partner_shipping_id
                if len(self.grouped_invoices) > 0
                else False
            )
            partner_id = (
                self.grouped_invoices[0].partner_id
                if len(self.grouped_invoices) > 0
                else False
            )

            company_id = (
                self.grouped_invoices[0].company_id
                if len(self.grouped_invoices) > 0
                else False
            )
            move_type = (
                self.grouped_invoices[0].move_type
                if len(self.grouped_invoices) > 0
                else False
            )
            amount_by_group = (
                self.grouped_invoices[0].amount_by_group
                if len(self.grouped_invoices) > 0
                else False
            )
            payment_reference = (
                self.grouped_invoices[0].payment_reference
                if len(self.grouped_invoices) > 0
                else False
            )
            min_date_invoice = min([inv.invoice_date for inv in self.grouped_invoices])
            currency_id = (
                self.grouped_invoices[0].currency_id
                if len(self.grouped_invoices) > 0
                else False
            )
            ...
            group_name = (
                self.grouped_invoices[0].name + "-group"
                if len(self.grouped_invoices) > 0
                else False
            )
            ...
            if self.payment_type == "iban":
                bank_account = sanitize_account_number(
                    partner_bank_id.l10n_ch_qr_iban or partner_bank_id.acc_number
                )
            else:
            ...
            delivery = (
                partner_shipping_id if partner_shipping_id != partner_id else False
            )
            ...
            params = {
                ...
                "amount_total": sum(
                    [
                        inv.amount_total if inv.amount_total else 0
                        for inv in self.grouped_invoices
                    ]
                ),
                "amount_untaxed": sum(
                    [
                        inv.amount_untaxed if inv.amount_untaxed else 0
                        for inv in self.grouped_invoices
                    ]
                ),
                "amount_tax": sum(
                    [
                        inv.amount_tax if inv.amount_tax else 0
                        for inv in self.grouped_invoices
                    ]
                ),
                "saleorder": orders,
                ...
                "invoice_lines": self.grouped_invoices.invoice_line_ids.filtered(
                    lambda r: not r.display_type
                ),
                "grouped_invoices": self.grouped_invoices,
                "lines_by_invoice": {
                    inv: inv.postfinance_invoice_line_ids()
                    for inv in self.grouped_invoices
                },
            ...
            for inv in self.grouped_invoices:
                ...
                if not date_due:
                    if inv.invoice_payment_term_id:
                        terms = inv.invoice_payment_term_id.compute(inv.amount_total)
                        if terms:
            ...
            params["first_due_date"] = min(
                [data_due_by_invoice[i] for i in data_due_by_invoice]
            )
            return params
