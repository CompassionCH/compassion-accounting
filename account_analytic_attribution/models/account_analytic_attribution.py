...
class AccountAttribution(models.Model):
    ...
    analytic_tag_id = fields.Many2one(
        "account.analytic.tag", "Analytic Tag", ondelete="cascade", readonly=False
    )
    account_distribution_line_ids = fields.One2many(
        "account.analytic.distribution.line",
        "attribution_id",
        "Distribution",
        required=True,
        readonly=False,
    ...
    def get_attribution(self, account_tag_ids, analytic_tag_ids, date):
        ...
        if account_tag_ids:
            domain += [
                "|",
                ("account_tag_id", "in", account_tag_ids),
        ...
        if analytic_tag_ids:
            domain += [
                "|",
                ("analytic_tag_id", "in", analytic_tag_ids),
    ...
    def perform_distribution(self, date_start=None, date_stop=None):
        ...
        tag_id = self.env.ref("account_analytic_attribution.tag_attribution").id
                ...
                if attribution_rule:
                    ...
                    for rule in attribution_rule.account_distribution_line_ids:
                        line = analytic_line_obj.create(
                            {
                                "name": "Analytic attribution for " + analytic.name,
    ...
    def _filter_analytic_lines_and_removed_old_ones(
        self, date_start, date_stop, tag_id
    ):
        analytic_line_obj = self.env["account.analytic.line"]
