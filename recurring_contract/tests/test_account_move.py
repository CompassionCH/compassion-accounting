...
class AccountInvoiceTestCase(AccountTestInvoicingCommon):
    ...
    def test_update_invoices(self):
        ...
        updt_val = {
            self.invoice.name: {
                "invoice_line_ids": [
                    (1, self.invoice.invoice_line_ids[0].id, {"price_unit": 20.0})
                ]
            }
        }
    ...
    def test_update_invoices_cancel_invoice(self):
        ...
        Asserts that the total amount is correct after the update and that
        the invoice state is 'cancel'
        """
        updt_val = {
            self.invoice.name: {
                "invoice_line_ids": [
                    (1, self.invoice.invoice_line_ids[0].id, {"price_unit": 0.0})
                ]
            }
        }
        self.invoice.update_open_invoices(updt_val)
    ...
    def test_update_invoices_post_invoice(self):
        ...
        Asserts that the total amount is correct after the update and
        that the invoice state is 'posted'
        """
        updt_val = {
            self.invoice.name: {
                "invoice_line_ids": [
                    (1, self.invoice.invoice_line_ids[0].id, {"price_unit": 100.0})
                ]
            }
        }
        self.invoice.update_open_invoices(updt_val)
    ...
    def test_build_invoice_data(self):
        ...
        result = self.invoice._build_invoices_data(
