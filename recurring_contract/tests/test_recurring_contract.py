...
class BaseContractTest(TransactionCase):
    ...
    def setUp(self):
        ...
        self.env["ir.config_parameter"].set_param(
            f"recurring_contract.do_generate_curr_month_{self.env.company.id}", "False"
        )
        self.env["ir.config_parameter"].set_param(
            f"recurring_contract.inv_block_day_{self.env.company.id}", 31
        )
    ...
    def _pay_invoice(self, invoice):
        ...
        # (the context simulate what's done in the web interface)
...
class TestRecurringContract(BaseContractTest):
    ...
    def test_contract_basic_workflow(self):
        ...
        Created in draft -> Waiting with invoices generated ->
        active by paying the invoices -> terminated
        """
        ...
        # Ensure the good number of moves are generated 2
        ...
        # We end a contract it should put it in state terminated
        ...
        invoices = contract.invoice_line_ids.mapped("move_id").filtered(
            lambda m: m.state == "posted" and m.payment_state == "not_paid"
        )
    ...
    def test_generate_invoice_data_coherency(self):
        ...
        for invoice in invoices:
            self.assertEqual(
                invoice.partner_id, contract.partner_id, "The partners doesn't match"
            )
            ...
            for invoice_line in invoice.invoice_line_ids:
                ...
                self.assertEqual(
                    invoice_line.product_id, contract.contract_line_ids[0].product_id
                )
                self.assertEqual(
                    invoice_line.quantity, contract.contract_line_ids[0].quantity
                )
                self.assertEqual(
                    invoice_line.price_unit, contract.contract_line_ids[0].amount
                )
        contract.write(
            {
        ...
        contract.group_id.write({"advance_billing_months": 3})
        invoices = contract.invoice_line_ids.mapped("move_id")
    ...
    def test_invoice_multiple_contract_one_pay_opt(self):
        ...
        for invoice in invoices:
            ...
            self.assertListEqual(
                invoice_lines.mapped("product_id").ids,
                contract_lines.mapped("product_id").ids,
            )
            self.assertListEqual(
                invoice_lines.mapped("quantity"), contract_lines.mapped("quantity")
            )
            self.assertListEqual(
                invoice_lines.mapped("price_unit"), contract_lines.mapped("amount")
            )
        # Terminating one contract (line should be erased of the open invoice
        contracts[0].action_contract_terminate()
        no_invoice = invoices.mapped("invoice_line_ids").filtered(
            lambda invl: invl.contract_id == contracts[0]
        )
    ...
    def test_invoice_create_contract_invoice_already_paid(self):
        ...
        self.assertListEqual(
            invoices_after.mapped("amount_total"), contracts.mapped("total_amount")
        )
    ...
    def test_invoice_create_contract_invoice_already_paid_do_gen_curr_month(self):
        ...
        """
        self.env["ir.config_parameter"].set_param(
            f"recurring_contract.do_generate_curr_month_{self.env.company.id}", "True"
        )
        contract = self.contract
        self.assertEqual(len(contract.contract_line_ids), 1)
        contract_2 = self.create_contract(
