...
class MandateStaffNotifSettings(models.TransientModel):
    ...
    do_generate_curr_month = fields.Boolean(
        ...
        help="Define if the invoices should generate the current month by default "
        "or the next month."
        "Ticked means we generate the current month and next month by default",
        default=True,
    )
    # Day to know when we should stop generating invoices
    # for a specific contract group on a contract creation
    inv_block_day = fields.Selection(
        selection="_day_selection",
        string="Invoices Blocked Day",
        help="If set, contracts created after this day will set their first invoice "
        "one month after. "
    ...
    def get_values(self):
        ...
        res["inv_block_day"] = self.get_param_multi_company(
            "recurring_contract.invoice_block_day"
        )
        res["do_generate_curr_month"] = (
    ...
    def set_values(self):
        self._set_param_multi_company(
            "recurring_contract.invoice_block_day", self.inv_block_day
        )
        self._set_param_multi_company(
