...
class ContractGroup(models.Model):
    ...
    def write(self, vals):
        ...
        if vals.get("invoice_suspended_until"):
            if fields.Date.from_string(vals["invoice_suspended_until"]) < datetime.today().date():
                raise UserError(_("The suspension of invoices has to be in the future ! (Invoice Suspended Until field)"))
    ...
    def _generate_invoices(self, invoicer):
        ...
        for group in self:
            ...
            month_interval = group.recurring_value * (1 if recurring_unit == "month" else 12)
            for inv_no in range(0 if curr_month else 1, group.advance_billing_months + 1, month_interval):
                ...
                if group.invoice_suspended_until and group.invoice_suspended_until > invoicing_date:
                    continue
                ...
                if open_invoice:
                    ...
                    already_paid_cl = self.env["account.move.line"].search([
                        ("date", "=", invoicing_date),
                    ...
                    open_invoice.write({
                        "invoice_line_ids": [
                            (0, 0, group.build_inv_line_data(invoicing_date=invoicing_date, contract_line=cl))
                    ...
                    # check if there is already an invoice for this partner and due date which has been cancelled
